...
void VRP_Graph::read_undirected_VRP(istream & fic){
 if (!fic){
    ...
    V_links.resize(nb_links);
		...
		fic>>V_nodes[0].y;
		V_nodes[0].num = 0;
		V_nodes[0].L_adjLinks.clear();
		V_nodes[0].weight=1;
		if (maxx<V_nodes[0].x) maxx= V_nodes[0].x;
		if (maxy<V_nodes[0].y) maxy= V_nodes[0].y;
		if (minx>V_nodes[0].x) minx= V_nodes[0].x;
		if (miny>V_nodes[0].y) miny= V_nodes[0].y;
		
		cout<<"0 "<<V_nodes[0].x<< " "<<V_nodes[0].y<<endl;
    for (i=1;i<nb_nodes;i++){
    	fic>>m1;
    	fic>>V_nodes[i].x;
			fic>>V_nodes[i].y;

			cout<<m1<<" "<<V_nodes[i].x<< " "<<V_nodes[i].y<<endl;
      V_nodes[i].num = i;
      ...
      V_nodes[i].weight=1;
		...
		for(it1=0;it1<nb_nodes;it1++){
			for (it2=it1+1;it2<nb_nodes;it2++){
				a=new C_link;
				a->num=cpt;
      	a->v1=it1;
      	a->v2=it2;
				a->length=lengthTSP(it1,it2);
	      V_nodes[it1].L_adjLinks.push_back(a);
	      V_nodes[it2].L_adjLinks.push_back(a);
	      V_links[cpt] = a;
				cpt++;
			}
		}
		fic>>m1;//m1=Demand_Section
		
		demand.resize(nb_nodes);
		for(it1=0;it1<nb_nodes;it1+=1){
			fic>>m1;
...
void VRP_Graph::read_directed_VRP(istream & fic){
 if (!fic){
    cout<<"File Error"<<endl;
  }else{
    int k,i,j;
      ...
      V_nodes[i].weight=1;
		...
		for(it1=0;it1<nb_nodes;it1++){
      	...
      	b->v2=it1;
				...
				b->length=a->length;
	      V_nodes[it1].L_adjLinks.push_back(a);
	      V_nodes[it2].L_adjLinks.push_back(b);
	      V_links[cpt] = a;
	      V_links[cpt+1] = b;
				cpt+=2;
			}
...
double C_Graph::Directed_MinimumCut(list<int>& W){
  int i;
  list<C_link *>::const_iterator it;
  lemon::ListDigraph::ArcMap<double> L_cost_int(L_GD);
  lemon::ListDigraph::NodeMap<bool> mincut(L_GD);
  double mincutvalue;
  ...
  for (i=0;i<nb_nodes;i++){
	...
	//cout<<"("<<L_GU.id((*it)->LGU_name)<<","<<(*it)->algo_cost<<") ";
	L_cost_int.set((*it)->LGD_name,(*it)->algo_cost*PREC);
    }
    }
  ...
  lemon::HaoOrlin<lemon::ListDigraph, lemon::ListDigraph::ArcMap<double> > L_NI(L_GD,L_cost_int);
  ...
  W.clear();
  for (i=0;i<nb_nodes;i++)
    if (mincut[V_nodes[i].LGD_name]) W.push_back(i);
  
  #ifdef OUTPUT_GRAPH  
  cout<<"MinCut value : "<<mincutvalue<<endl;
  cout<<"MinCut induced by : ";
  for (i=0;i<nb_nodes;i++)
    if (mincut[V_nodes[i].LGD_name]) cout<<i<<" ";
  cout<<endl;
  #endif
